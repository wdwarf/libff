...
Log::Log() : m_logLevel(LogLevel::Info), m_lineNumber(0) {}
...
LoggerPtr Log::CreateLogger(const std::string &m_module, LogLevel logLevel,
  ...
  LoggerPtr logger = make_shared<Logger>(m_module, logLevel, threading);
...
Log &Log::done() {
  ...
  if (logInfo.empty()) return *this;

  this->m_stream.clear();
  this->m_stream.str("");

  this->log(LogInfo(this->m_logLevel, logInfo, this->m_module, Timestamp(),
                    m_fileName, this->m_functionName, this->m_lineNumber));

  return *this;
}

const LogLevel &Log::getLogLevel() const { return m_logLevel; }
...
const std::string &Log::getFileName() const { return m_fileName; }
...
const std::string &Log::getFunctionName() const { return m_functionName; }
...
unsigned int Log::getLineNumber() const { return m_lineNumber; }
