...
TEST(MessageBusTest, MessageBusTest) {
  ...
  client2.on(102, [](const MsgBusPackage& pkg) {});
  client2.on(1000, [](const MsgBusPackage& pkg) {});
  client2.start(6600, "127.0.0.1", 0);
  client2.on(2000, [](const MsgBusPackage& pkg) {});

  thread([&client2, &client1]() {
    do {
      this_thread::sleep_for(chrono::milliseconds(100));
      cout << "start req" << endl;

      auto promise = client2.req(101, 1);
